...
var buffer_last_window_id = chrome.windows.WINDOW_ID_CURRENT;
chrome.windows.onFocusChanged.addListener(function(windowId){
  chrome.windows.getLastFocused((window) => buffer_last_window_id = window.id);
  //console.log(buffer_last_window_id);
...
chrome.commands.onCommand.addListener((command) => {
  if (command == "move"){
    ...
    async_move(tab_id, last_window_id);
  }
});

//moving the tab to another chrome window
function move(tabId, last_window_id) {
  chrome.tabs.move(tabId, {windowId: last_window_id});
  console.log('Success.');
}
